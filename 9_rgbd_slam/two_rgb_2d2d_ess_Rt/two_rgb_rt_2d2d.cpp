...
int main(int argc, char **argv) {
    ...
    for (DMatch m: matches) {
        // 像素点转换为成像平面的点，在转换为归一化平面的点
        Point2d pt1 = pixel2cam(keypoints_1[m.queryIdx].pt, K);
        Mat y1 = (Mat_<double>(3, 1) << pt1.x, pt1.y, 1); // 构建归一化平面上面的点
...
void find_feature_matches(const Mat &img_1, const Mat &img_2,
    ...
    Mat descriptors_1, descriptors_2;  //-- 初始化
    Ptr<FeatureDetector> detector = ORB::create();  // used in OpenCV3
    Ptr<DescriptorExtractor> descriptor = ORB::create();  // used in OpenCV3
    // Ptr<FeatureDetector> detector = FeatureDetector::create ( "ORB" );　// use this if you are in OpenCV2
    // Ptr<DescriptorExtractor> descriptor = DescriptorExtractor::create ( "ORB" );　// use this if you are in OpenCV2
...
void pose_estimation_2d2d(std::vector<KeyPoint> keypoints_1,
    ...
    // 根据匹配特征点matches的索引,在特征点keypoints_1中搜索,将匹配的特征点存储到points1,points2
