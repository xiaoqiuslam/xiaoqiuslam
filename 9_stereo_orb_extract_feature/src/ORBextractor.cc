...
#undef GET_VALUE
    ...
    ORBextractor::ORBextractor(int _nfeatures, float _scaleFactor, int _nlevels, int _iniThFAST, int _minThFAST)
        ...
        // std::vector<cv::Mat> mvImagePyramid; 这个vector就是传说中的图像金字塔了
        mvImagePyramid.resize(nlevels);
        // std::vector<int> mnFeaturesPerLevel; 这个vector存储图像金字塔每层需要提取的特征点个数
        mnFeaturesPerLevel.resize(nlevels);
        // 图像缩放系数的倒数 double scaleFactor;
        float factor = 1.0f / scaleFactor;
        // https://zhuanlan.zhihu.com/p/61738607 OpenCV里不按面积算按边长算
        // 计算第0层图像也就是原始图像需要提取特征点的数目
        float nDesiredFeaturesPerScale =
                nfeatures * (1 - factor) /
                (1 - (float) pow((double) factor, (double) nlevels));
        int sumFeatures = 0;
        for (int level = 0; level < nlevels - 1; level++) {
            // cvRound 取整,将第1层提取到的特征点数目取整然后存储到vector->mnFeaturesPerLevel
            // 这个vector存储图像金字塔每层图像需要提取的特征点个数
        ...
        std::cout << pattern0[0] << endl;
        std::cout << pattern0[1] << endl;
        std::cout << pattern0[2] << endl;
        std::cout << pattern0[3] << endl;
        /**
         * 8,  -3,  9,   5
         * 4,   2,  7,   -12
         * [8, -3]
        ...
        *   umax[13]: 8
        *   umax[12]: 9
        *   umax所有16个数值依次是：15 15 15 15 14 14 14 13 13 12 11 10 9 8 6 3
        */
    }
 ...
 */
    ...
    int ORBextractor::operator()(InputArray _image, InputArray _mask,
        ...
        if (nkeypoints == 0)
            ...
            descriptors = _descriptors.getMat();  // descriptors目前为空
        }

        _keypoints = vector<cv::KeyPoint>(nkeypoints);

        //备注：KEYPOINT 1 与　KEYPOINT 2的值相等，KEYPOINT
        //3为KEYPOINT1　的ｓｃａｌｅ倍
        int offset = 0;
